...
export async function extractVideoUrl(pageUrl, fromJid = null) {
  ...
  if (urlLower.endsWith('.mp4') || urlLower.endsWith('.mkv') || urlLower.endsWith('.webm') || urlLower.endsWith('.avi') ||
      urlLower.endsWith('.apk') || urlLower.endsWith('.zip') || urlLower.endsWith('.rar') || urlLower.endsWith('.7z') ||
      urlLower.endsWith('.pdf') || urlLower.endsWith('.exe')) {
...
export async function extract9Mod(targetUrl) {
  ...
  if (!pageUrl.includes('/download/')) {
    const pageRes = await gotScraping.get({
      url: pageUrl,
      cookieJar,
      headerGeneratorOptions: { devices: ['desktop'], locales: ['tr-TR', 'en-US'], operatingSystems: ['windows'] }
    });
    const $ = cheerio.load(pageRes.body);
