...
export async function extractSezonlukdizi(pageUrl) {
  console.log(`[SezonlukDizi Extractor] Using shared browser for: ${pageUrl}`);
  ...
  try {
    console.log("[SezonlukDizi Extractor] Navigating to page...");
    ...
    const pageTitle = await page.evaluate(() => {
      ...
      return h1 ? h1.innerText.replace(/izle/i, '').trim() : 'SezonlukDizi';
    });
    ...
    console.log(`[SezonlukDizi Extractor] Found ${iframes.length} iframes.`, iframes);
    ...
    for (const embedUrl of supportedIframes) {
      ...
      if (videoUrl) {
        console.log(`[SezonlukDizi Extractor] Successfully extracted video: ${videoUrl}`);
        await page.close();
        return {
          title: pageTitle,
          url: videoUrl,
    ...
    console.error(`[SezonlukDizi Extractor] Error: ${err.message}`);
  } finally {
    await page.close();
  }

  throw new Error('SezonlukDizi indirme bağlantısı alınamadı. Desteklenen bir video kaynağı (Vidmoly/Filemoon) bulunamadı.');
}
